...
function initUnlock(){
  ...
  // If helper exists, focus input when tapped (mobile-friendly)
  if(focusHelper){
...
/* -------------------------
   Letter page behavior (hidden letter -> reveal letter -> reveal message & play)
   ------------------------- */
function initLetterPage(){
  ...
  if(!openLetterBtn || !letterFull || !flowerMessage) return;
  ...
  // Prepare: set message text but keep hidden until user clicks the letter image (and ensure button exists)
  function prepareForRevealSteps(){
    ...
    // When "Open Letter" button is clicked we show the full letter image (still blurred),
    // and attach the image-click handler to reveal the message and play music.
    openLetterBtn.addEventListener('click', () => {
      // show the full letter image
      letterFull.style.display = '';
      openLetterBtn.style.display = 'none';
      // attach the click/keyboard handlers to the letter image (only once)
  ...
  function revealMessageAndPlay(){
    // unblur the image (if it is blurred)
  ...
  // Wait for unlock, then prepare the button behavior
  if(isUnlocked()){
    prepareForRevealSteps();
  } else {
    document.addEventListener('site-unlocked', prepareForRevealSteps, { once: true });
  }
